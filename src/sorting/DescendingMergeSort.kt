...
fun main(args: Array<String>) {
    ...
    mergeSortDesc(array, 0, array.size) // sorting.merge sort function to sort the given array in descending order


    array.printArray() // array after sorting

    //the sorting.merge sort is not an in-place algorithm
    //O (n log n)(base 2) time complexity
    //stable algorithm
}


fun mergeSortDesc(input: Array<Int>, start: Int, end: Int) {
    ...
    mergeSortDesc(input, start, mid)  // split recursively the left side of the array
    mergeSortDesc(input, mid, end) //split recursively the right section oft the array

    mergeDesc(input, start, mid, end)  // sorting.merge the left and right side of the array making sure they are sorted
}

fun mergeDesc(input: Array<Int>, start: Int, mid: Int, end: Int) {
    ...
    * their sorted positions**/
    ...
    /*if we have not fully traversed the left array sorting.partition or the right
    * sorting.partition keep looping*/
    while (i < mid && j < end) {

        /*increase temp index after each copy*/
        temp[tempIndex++] = when {
            input[i] >= input[j] -> input[i++] //if input at i is greater than input at j copy input at i and increase the value of i by 1
            else -> input[j++] //else copy the input at j since is greater than input at i and increase the value of j by 1
        }
    }

    /*copy the element from the left sorting.partition into the final destination actual array
     *if mid - i equals to zero then there are no elements to copy from the left side
     * any elements left in right sorting.partition will already be in their sorted position
